...
![Carpeta contorno](./image1.png){width="0.23950021872265967in"
height="0.23950021872265967in"} **tutoriaApp3**

![](./image3.png){width="1.458536745406824in"
...
    \$password = \"\"; //contraseña para la conexión (vacía por defecto
en desarrollo)
...
        //en caso de error en la conexión, terminar el script y mostrar
el mensaje de error
...
     \* \@return bool True si la autenticación es exitosa, false en caso
contrario
...
        \$stmt = \$this-\>conn-\>prepare(\"SELECT nombre, pass, activo
...
        // Verificar que el usuario exista, que la contraseña coincida y
que esté activo
...
        \$stmt = \$this-\>conn-\>prepare(\"SELECT rol FROM usuarios
WHERE id = :id\");
...
        \$stmt = \$this-\>conn-\>prepare(\"SELECT id FROM usuarios WHERE
nombre = :nombre\");
...
     \* \@return bool True si el registro es exitoso, false si el
usuario ya existe

     \*/

    public function register(\$nombre, \$pass, \$foto =
\'images/photo.png\') {
...
        \$stmt = \$this-\>conn-\>prepare(\"INSERT INTO usuarios (nombre,
pass, foto) VALUES (:nombre, :pass, :foto)\");
...
    public function registerWithRole(\$nombre, \$pass, \$rol, \$foto =
\'images/photo.png\') {
...
        \$stmt = \$this-\>conn-\>prepare(\"INSERT INTO usuarios (nombre,
pass, rol, foto) VALUES (:nombre, :pass, :rol, :foto)\");
...
            SELECT u.id, u.nombre, u.rol, u.activo, u.foto, CASE WHEN
t.id IS NOT NULL THEN \'Sí\' ELSE \'No\' END AS es_tutor
...
        // Primero eliminar registros relacionados en tutores y
tutor_asignatura

        \$stmt = \$this-\>conn-\>prepare(\"DELETE FROM tutor_asignatura
WHERE tutor_id IN (SELECT id FROM tutores WHERE user_id = :user_id)\");

        \$stmt-\>bindParam(\':user_id\', \$userId);

        \$stmt-\>execute();

        \$stmt = \$this-\>conn-\>prepare(\"DELETE FROM tutores WHERE
user_id = :user_id\");
...
        \$stmt = \$this-\>conn-\>prepare(\"DELETE FROM solicitudes WHERE
user_id = :user_id OR tutor_id IN (SELECT id FROM tutores WHERE user_id
= :user_id)\");
...
        \$stmt = \$this-\>conn-\>prepare(\"SELECT id, nombre, rol,
activo, foto FROM usuarios WHERE id = :id\");
...
    public function updateUser(\$userId, \$nombre, \$pass = null, \$foto
= null) {
...
nombre = :nombre, pass = :pass, foto = :foto WHERE id = :id\");
...
            \$stmt = \$this-\>conn-\>prepare(\"UPDATE usuarios SET
nombre = :nombre, pass = :pass WHERE id = :id\");
...
        \$stmt = \$this-\>conn-\>prepare(\"UPDATE usuarios SET activo =
:activo WHERE id = :id\");
...
SELECT t.id, t.user_id, u.nombre, t.descripcion, GROUP_CONCAT(a.nombre
SEPARATOR \', \') AS asignaturas, u.foto
...
\$stmt = \$this-\>conn-\>prepare(\"SELECT id, descripcion FROM tutores
...
\$stmt = \$this-\>conn-\>prepare(\"UPDATE tutores SET descripcion =
...
\$stmt = \$this-\>conn-\>prepare(\"INSERT INTO tutor_asignatura
(tutor_id, asignatura_id) VALUES (:tutor_id, :asignatura_id)\");
...
\$stmt = \$this-\>conn-\>prepare(\"DELETE FROM tutor_asignatura WHERE
tutor_id = :tutor_id AND asignatura_id = :asignatura_id\");
...
\$stmt = \$this-\>conn-\>prepare(\"INSERT INTO tutores (user_id,
descripcion) VALUES (:user_id, :descripcion)\");
...
\$stmt = \$this-\>conn-\>prepare(\"SELECT id FROM asignaturas WHERE
nombre = :nombre\");
...
\$stmt = \$this-\>conn-\>prepare(\"INSERT INTO asignaturas (nombre)
VALUES (:nombre)\");
...
\$stmt = \$this-\>conn-\>prepare(\"SELECT id, nombre FROM
asignaturas\");
...
\$stmt = \$this-\>conn-\>prepare(\"UPDATE asignaturas SET nombre =
:nombre WHERE id = :id\");
...
\$stmt = \$this-\>conn-\>prepare(\"DELETE FROM asignaturas WHERE id =
:id\");
...
\$stmt = \$this-\>conn-\>prepare(\"SELECT id FROM tutores WHERE user_id
= :user_id\");
...
        public function createRequest (\$userId, \$tutorId,
\$asignaturaId) {

   // Verificar si ya existe una solicitud pendiente o aceptada para el
mismo usuario, tutor y asignatura

            \$stmt = \$this-\>conn-\>prepare(\"SELECT id FROM
solicitudes WHERE user_id = :user_id AND tutor_id = :tutor_id AND
asignatura_id = :asignatura_id AND estado IN (\'pendiente\',
\'aceptada\')\");
...
            \$stmt = \$this-\>conn-\>prepare(\"INSERT INTO solicitudes
(user_id, tutor_id, asignatura_id) VALUES (:user_id, :tutor_id,
:asignatura_id)\");
...
        \$stmt = \$this-\>conn-\>prepare(\"UPDATE solicitudes SET estado
= \'aceptada\' WHERE id = :id\");
...
     \* Obtiene todas las solicitudes pendientes para que el admin las
revise.
...
            SELECT s.id, u.nombre AS usuario, tu.nombre AS tutor,
a.nombre AS asignatura, s.fecha_solicitud
...
     \* Obtiene todas las solicitudes aceptadas para que el admin las
revise.
...
     \* Obtiene todas las solicitudes rechazadas para que el admin las
revise.
...
            \$stmt = \$this-\>conn-\>prepare(\"SELECT s.id, u.nombre AS
tutor, a.nombre AS asignatura, s.estado, s.fecha_solicitud,
...
            // Verificar que la solicitud pertenece al usuario y no ha
sido aceptada

            \$stmt = \$this-\>conn-\>prepare(\"SELECT estado,
TIMESTAMPDIFF(HOUR, fecha_solicitud, NOW()) AS horas_transcurridas FROM
solicitudes WHERE id = :id AND user_id = :user_id\");
...
                return false; // Solicitud no encontrada o no pertenece
al usuario
...
                return false; // No se puede cancelar una solicitud
aceptada
...
                return false; // No se puede cancelar después de 24
horas
...
            \$stmt = \$this-\>conn-\>prepare(\"DELETE FROM solicitudes
WHERE id = :id\");
...
        // Crear instancia del modelo de usuario para acceder a la base
de datos
...
                    \$\_SESSION\[\'login_error\'\] = \"Tu cuenta está
desactivada. Contacta al administrador para reactivarla.\";
...
                \$\_SESSION\[\'rol\'\] =
\$this-\>userModel-\>getUserRole(\$\_SESSION\[\'user_id\'\]);
...
                // Si falla, mostrar mensaje de error y recargar vista
home

                \$\_SESSION\[\'login_error\'\] = \"Usuario o contraseña
incorrectos.\";
...
                // Si es exitoso, mostrar mensaje de éxito y redirigir a
home

                \$\_SESSION\[\'register_success\'\] = \"Registro
exitoso. Puede iniciar sesión.\";
...
                // Si falla, mostrar mensaje de error y redirigir a
register

                \$\_SESSION\[\'register_error\'\] = \"El usuario ya
existe. Intente con otro nombre de usuario.\";
...
            // Si no es POST, mostrar formulario registro con lista de
tutores
...
            if
(\$this-\>userModel-\>updateUser(\$\_SESSION\[\'user_id\'\], \$nombre,
\$pass, \$foto)) {

                \$\_SESSION\[\'nombre\'\] = \$nombre;

                \$\_SESSION\[\'profile_success\'\] = \"Perfil
actualizado exitosamente.\";

            } else {

                \$\_SESSION\[\'profile_error\'\] = \"Error al actualizar
el perfil. El nombre de usuario ya existe.\";
...
       
\$this-\>userModel-\>toggleUserStatus(\$\_SESSION\[\'user_id\'\],
\$newStatus);

        if (\$newStatus) {

            \$\_SESSION\[\'profile_success\'\] = \"Cuenta reactivada
exitosamente.\";

        } else {

            \$\_SESSION\[\'profile_success\'\] = \"Cuenta desactivada
exitosamente.\";
...
-   **controllers/HomeController.php**: Muestra la página principal con
...
            \$\_SESSION\[\'error\'\] = \"Tu cuenta está desactivada. No
puedes registrarte como tutor.\";
...
        // Si ya es tutor, obtener asignaturas actuales para marcar
checkboxes

        \$isTutor =
\$this-\>tutorModel-\>isTutor(\$\_SESSION\[\'user_id\'\]);
...
            \$tutorId =
\$this-\>tutorModel-\>getTutorIdByUserId(\$\_SESSION\[\'user_id\'\]);

            \$currentAsignaturas =
\$this-\>tutorModel-\>getTutorAsignaturas(\$tutorId);
...
            \$tutor =
\$this-\>tutorModel-\>getTutorByUserId(\$\_SESSION\[\'user_id\'\]);
...
           
\$this-\>tutorModel-\>registerTutor(\$\_SESSION\[\'user_id\'\],
...
            if
(\$this-\>requestModel-\>createRequest(\$\_SESSION\[\'user_id\'\],
\$tutorId, \$asignaturaId)) {
...
                \$\_SESSION\[\'request_error\'\] = \"Ya tienes una
solicitud activa para este tutor y asignatura.\";
...
        \$requests =
\$this-\>requestModel-\>getUserRequests(\$\_SESSION\[\'user_id\'\]);
...
                \$\_SESSION\[\'cancel_success\'\] = \"Solicitud
cancelada exitosamente.\";

            } else {

                \$\_SESSION\[\'cancel_error\'\] = \"No se pudo cancelar
la solicitud. Puede que ya haya sido aceptada o haya pasado más de 24
horas.\";
...
        \$pendingRequests =
\$this-\>requestModel-\>getPendingRequests();

        \$acceptedRequests =
\$this-\>requestModel-\>getAcceptedRequests();
...
            \$asignatura =
\$this-\>tutorModel-\>getAsignaturaById(\$\_GET\[\'asignatura_id\'\]);
...
            \$asignatura =
\$this-\>tutorModel-\>getAsignaturaById(\$id);
...
            if (\$this-\>userModel-\>registerWithRole(\$nombre, \$pass,
\$rol, \$foto)) {
...
                if
(\$this-\>userModel-\>deleteUser(\$\_GET\[\'user_id\'\])) {
...
            \$user =
\$this-\>userModel-\>getUserById(\$\_GET\[\'user_id\'\]);

            \$newStatus = \$user\[\'activo\'\] ? 0 : 1;

           
\$this-\>userModel-\>toggleUserStatus(\$\_GET\[\'user_id\'\],
\$newStatus);
...
            \<a href=\"index.php?action=admin_dashboard\"\>Panel de
Admin\</a\> \|

        \<?php else: ?\>

            \<a href=\"index.php?action=register_tutor\"\>Registrarse
como Tutor\</a\> \|

            \<a href=\"index.php?action=my_requests\"\>Mis
Solicitudes\</a\> \|
...
        \<?php if (isset(\$\_SESSION\[\'request_error\'\])) { echo \"\<p
style=\'color:red;\'\>\".\$\_SESSION\[\'request_error\'\].\"\</p\>\";
unset(\$\_SESSION\[\'request_error\'\]); } ?\>

    \<?php else: ?\>

        \<div style=\"float: right; width: 300px; border: 1px solid
...
\"\<p
style=\'color:red;\'\>\".\$\_SESSION\[\'login_error\'\].\"\</p\>\";
unset(\$\_SESSION\[\'login_error\'\]); } ?\>

            \<?php if (isset(\$\_SESSION\[\'register_success\'\])) {
echo \"\<p
style=\'color:green;\'\>\".\$\_SESSION\[\'register_success\'\].\"\</p\>\";
unset(\$\_SESSION\[\'register_success\'\]); } ?\>

            \<?php if (isset(\$\_SESSION\[\'register_error\'\])) { echo
\"\<p
style=\'color:red;\'\>\".\$\_SESSION\[\'register_error\'\].\"\</p\>\";
...
            \<p\>\<a
href=\"index.php?action=register\"\>Registrarse\</a\>\</p\>
...
                \<img src=\"\<?php echo \$tutor\[\'foto\'\]; ?\>\"
alt=\"Foto de \<?php echo \$tutor\[\'nombre\'\]; ?\>\" style=\"width:
100px; height: 100px; border-radius: 50%;\"\>\<br\>

                \<strong\>\<?php echo \$tutor\[\'nombre\'\];
?\>\</strong\>\<br\>

                Descripción: \<?php echo \$tutor\[\'descripcion\'\];
?\>\<br\>

                Asignaturas: \<?php echo \$tutor\[\'asignaturas\'\];
?\>\<br\>

                \<?php if (isset(\$\_SESSION\[\'user_id\'\])): ?\>

                    \<?php if (\$tutor\[\'user_id\'\] ==
\$\_SESSION\[\'user_id\'\]): ?\>

                        \<em\>No puedes solicitarte tutoría a ti
mismo\</em\>

                    \<?php else: ?\>

                        \<a
href=\"index.php?action=request_tutoria&tutor_id=\<?php echo
\$tutor\[\'id\'\]; ?\>\"\>Solicitar Tutor\</a\>
...
    \<p\>\<a href=\"index.php?action=home\"\>Volver al
Inicio\</a\>\</p\>
...
    \<div style=\"float: right; width: 300px; border: 1px solid #ccc;
...
        \<p style=\"color:red; font-weight:bold;\"\>Tu cuenta está
DESACTIVADA. Contacta al administrador para reactivarla.\</p\>

    \<?php endif; ?\>

    \<?php if (isset(\$\_SESSION\[\'profile_success\'\])) { echo \"\<p
style=\'color:green;\'\>\".\$\_SESSION\[\'profile_success\'\].\"\</p\>\";
unset(\$\_SESSION\[\'profile_success\'\]); } ?\>

    \<?php if (isset(\$\_SESSION\[\'profile_error\'\])) { echo \"\<p
style=\'color:red;\'\>\".\$\_SESSION\[\'profile_error\'\].\"\</p\>\";
unset(\$\_SESSION\[\'profile_error\'\]); } ?\>

    \<form method=\"post\"
action=\"index.php?action=edit_profile_process\"
enctype=\"multipart/form-data\"\>
...
        \<label for=\"pass\"\>Nueva Contraseña (dejar vacío para no
cambiar):\</label\>
...
actual\" style=\"width: 100px; height: 100px;\"\>\<br\>
...
        \<p\>Estado actual: \<?php echo \$user\[\'activo\'\] ?
\'Activa\' : \'Desactivada\'; ?\>\</p\>

        \<form method=\"post\"
action=\"index.php?action=toggle_account_status\"\>

            \<button type=\"submit\"\>\<?php echo \$user\[\'activo\'\] ?
\'Desactivar Cuenta\' : \'Reactivar Cuenta\'; ?\>\</button\>
...
    \<h2\>\<?php echo \$isTutor ? \'Actualizar Perfil de Tutor\' :
\'Registrarse como Tutor\'; ?\>\</h2\> \<!\-- Encabezado principal \--\>

    \<form method=\"post\"
action=\"index.php?action=register_tutor_process\"\> \<!\-- Formulario
POST para procesar registro \--\>

        \<label for=\"descripcion\"\>Descripción:\</label\>\<br\> \<!\--
Etiqueta para descripción \--\>

        \<textarea name=\"descripcion\" required\>\<?php echo \$isTutor
? htmlspecialchars(\$tutor\[\'descripcion\'\] ?? \'\') : \'\';
?\>\</textarea\>\<br\> \<!\-- Campo de texto para descripción \--\>

        \<label\>Asignaturas:\</label\>\<br\> \<!\-- Etiqueta para
asignaturas \--\>

        \<?php foreach (\$asignaturas as \$asignatura): ?\> \<!\-- Bucle
para cada asignatura \--\>

            \<input type=\"checkbox\" name=\"asignaturas\[\]\"
value=\"\<?php echo \$asignatura\[\'id\'\]; ?\>\" \<?php echo \$isTutor
&& in_array(\$asignatura\[\'id\'\], \$currentIds) ? \'checked\' : \'\';
?\>\> \<?php echo \$asignatura\[\'nombre\'\]; ?\>\<br\> \<!\-- Checkbox
para seleccionar asignatura \--\>

        \<?php endforeach; ?\> \<!\-- Fin bucle \--\>

        \<input type=\"submit\" value=\"\<?php echo \$isTutor ?
\'Actualizar Perfil\' : \'Registrarse como Tutor\'; ?\>\"\> \<!\-- Botón
de envío \--\>

    \</form\> \<!\-- Fin formulario \--\>

    \<a href=\"index.php?action=home\"\>Volver\</a\> \<!\-- Enlace para
volver \--\>
...
>
>     \<?php if (empty(\$asignaturas)): ?\> \<!\-- Verificar si hay
> asignaturas disponibles \--\>
>
>         \<p\>Este tutor no tiene asignaturas disponibles para
> solicitar tutoría.\</p\> \<!\-- Mensaje si no hay asignaturas \--\>
>
>         \<a href=\"index.php?action=home\"\>Volver\</a\> \<!\-- Enlace
> para volver \--\>
>
>     \<?php else: ?\> \<!\-- Si hay asignaturas \--\>
>
>         \<form method=\"post\"
> action=\"index.php?action=request_tutoria_process\"\> \<!\--
> Formulario POST para procesar solicitud \--\>
>
>             \<input type=\"hidden\" name=\"tutor_id\" value=\"\<?php
> echo \$\_GET\[\'tutor_id\'\]; ?\>\"\> \<!\-- Campo oculto con ID del
> tutor \--\>
>
>             \<label for=\"asignatura_id\"\>Asignatura:\</label\>
> \<!\-- Etiqueta para asignatura \--\>
>
>             \<select name=\"asignatura_id\" required\> \<!\-- Select
> para elegir asignatura \--\>
>
>                 \<?php foreach (\$asignaturas as \$asignatura): ?\>
> \<!\-- Bucle para cada asignatura \--\>
>
>                     \<option value=\"\<?php echo
> \$asignatura\[\'id\'\]; ?\>\"\>\<?php echo \$asignatura\[\'nombre\'\];
> ?\>\</option\> \<!\-- Opción de asignatura \--\>
>
>                 \<?php endforeach; ?\> \<!\-- Fin bucle \--\>
>
>             \</select\>\<br\> \<!\-- Salto de línea \--\>
>
>             \<input type=\"submit\" value=\"Solicitar\"\> \<!\-- Botón
> de envío \--\>
>
>         \</form\> \<!\-- Fin formulario \--\>
>
...
    \<?php if (isset(\$\_SESSION\[\'cancel_success\'\])) { echo \"\<p
style=\'color:green;\'\>\".\$\_SESSION\[\'cancel_success\'\].\"\</p\>\";
unset(\$\_SESSION\[\'cancel_success\'\]); } ?\>

    \<?php if (isset(\$\_SESSION\[\'cancel_error\'\])) { echo \"\<p
style=\'color:red;\'\>\".\$\_SESSION\[\'cancel_error\'\].\"\</p\>\";
unset(\$\_SESSION\[\'cancel_error\'\]); } ?\>

    \<?php

    \$pending = array_filter(\$requests, function(\$r) { return
\$r\[\'estado\'\] === \'pendiente\'; });

    \$accepted = array_filter(\$requests, function(\$r) { return
\$r\[\'estado\'\] === \'aceptada\'; });

    \$rejected = array_filter(\$requests, function(\$r) { return
\$r\[\'estado\'\] === \'rechazada\'; });
...
                Asignatura: \<?php echo \$request\[\'asignatura\'\];
?\>\<br\>
...
                    \<form method=\"POST\"
action=\"index.php?action=cancel_request\" style=\"display:inline;\"\>

                        \<input type=\"hidden\" name=\"request_id\"
value=\"\<?php echo \$request\[\'id\'\]; ?\>\"\>

                        \<button type=\"submit\" onclick=\"return
confirm(\'¿Estás seguro de cancelar esta solicitud?\')\"\>Cancelar
Solicitud\</button\>
...
                    \<em\>No se puede cancelar después de 24
horas\</em\>
...
    \<p\>Bienvenido, \<?php echo \$\_SESSION\[\'nombre\'\]; ?\> (Admin)
\| \<a href=\"index.php?action=logout\"\>Cerrar Sesión\</a\>\</p\>

    \<h3\>Gestión de Usuarios\</h3\>

    \<a href=\"index.php?action=register_user\"\>Registrar Nuevo
Usuario\</a\>

    \<h3\>Gestión de Asignaturas\</h3\>

    \<a href=\"index.php?action=add_asignatura\"\>Agregar Nueva
Asignatura\</a\>
...
                \<a
href=\"index.php?action=edit_asignatura&asignatura_id=\<?php echo
\$asignatura\[\'id\'\]; ?\>\"\>Editar\</a\>

                \<a
href=\"index.php?action=delete_asignatura&asignatura_id=\<?php echo
\$asignatura\[\'id\'\]; ?\>\" onclick=\"return confirm(\'¿Estás seguro
de eliminar esta asignatura?\')\"\>Eliminar\</a\>
...
alt=\"Foto de \<?php echo \$usuario\[\'nombre\'\]; ?\>\" style=\"width:
50px; height: 50px; border-radius: 50%; margin-right: 10px;\"\>

                \<?php echo \$usuario\[\'nombre\'\]; ?\> - Rol: \<?php
echo \$usuario\[\'rol\'\]; ?\> - Es Tutor: \<?php echo
\$usuario\[\'es_tutor\'\]; ?\>

                \<?php if (\$usuario\[\'activo\'\] == 0): ?\>

                    \<span style=\"color:red;
font-weight:bold;\"\>(DESACTIVADO)\</span\>
...
                    \<a
href=\"index.php?action=toggle_user_status&user_id=\<?php echo
\$usuario\[\'id\'\]; ?\>\"\>\<?php echo \$usuario\[\'activo\'\] ?
\'Desactivar\' : \'Activar\'; ?\>\</a\> \|

                    \<a
href=\"index.php?action=delete_user&user_id=\<?php echo
\$usuario\[\'id\'\]; ?\>\" onclick=\"return confirm(\'¿Estás seguro de
eliminar este usuario?\')\"\>Eliminar\</a\>
...
                \<?php echo \$tutor\[\'nombre\'\]; ?\> - Descripción:
\<?php echo \$tutor\[\'descripcion\'\]; ?\> - Asignaturas: \<?php echo
\$tutor\[\'asignaturas\'\]; ?\>
...
                Usuario: \<?php echo \$request\[\'usuario\'\]; ?\> -
Tutor: \<?php echo \$request\[\'tutor\'\]; ?\> - Asignatura: \<?php echo
\$request\[\'asignatura\'\]; ?\> - Fecha: \<?php echo
\$request\[\'fecha_solicitud\'\]; ?\>

                \<a
href=\"index.php?action=accept_request&request_id=\<?php echo
\$request\[\'id\'\]; ?\>\"\>Aceptar\</a\>

                \<a
href=\"index.php?action=reject_request&request_id=\<?php echo
\$request\[\'id\'\]; ?\>\" onclick=\"return confirm(\'¿Estás seguro de
rechazar esta solicitud?\')\"\>Rechazar\</a\>
...
    \<form method=\"post\"
action=\"index.php?action=add_asignatura_process\"\>
...
    \<form method=\"post\"
action=\"index.php?action=edit_asignatura_process\"\>

        \<input type=\"hidden\" name=\"asignatura_id\" value=\"\<?php
echo \$asignatura\[\'id\'\]; ?\>\"\>

        \<label for=\"nombre\"\>Nombre de la Asignatura:\</label\>

        \<input type=\"text\" name=\"nombre\" value=\"\<?php echo
htmlspecialchars(\$asignatura\[\'nombre\'\]); ?\>\" required\>
...
    \<form method=\"post\"
action=\"index.php?action=register_user_process\"
enctype=\"multipart/form-data\"\>
